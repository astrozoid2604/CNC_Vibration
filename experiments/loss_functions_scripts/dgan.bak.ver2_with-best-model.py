...
class DGAN:
    ...
    def _train(
        ...
        # Saving best model into a model file
        ...
        best_model_filename      = SLname + BSname + Ename + ANLname + ANUname + FNLname + FNUname + GLRname + DLRname + ADLRname + '_best.model'
        best_model_filepath      = self.cwd_path + best_model_filename

        lowest_total_gendis_loss = float('inf')
        new_best_model_found     = False
        best_model_batch_idx     = ""
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, real_batch in enumerate(loader):
                ...
                if np.abs(generator_loss) + np.abs(discriminator_loss) < lowest_total_gendis_loss:
                    lowest_total_gendis_loss = total_gendis_loss
                    new_best_model_found     = True
                    best_model_batch_idx     = batch_idx
                #logger.info(f"\t\tbatch_idx: {batch_idx}, gen_loss: {generator_loss:.3e}, dis_loss: {discriminator_loss:.3e}, tot_loss: {total_gendis_loss:.3e}")
                ...
                if progress_callback is not None:
                    ...
                    #    )
                    #)
                    progress_callback.add_scalar('Generator Loss',     generator_loss,     epoch*len(loader)+batch_idx)
                    progress_callback.add_scalar('Discriminator Loss', discriminator_loss, epoch*len(loader)+batch_idx)
                    progress_callback.add_scalar('Total Loss',         total_gendis_loss,  epoch*len(loader)+batch_idx)
            
            if new_best_model_found:
                logger.info(f"\nNew best model found at Epoch {epoch}/{self.config.epochs} and at batch_idx {best_model_batch_idx}/{len(loader)} with total_gendis_loss {lowest_total_gendis_loss:.4e}")
                logger.info(f"New best model filepath: {best_model_filepath}\n")
                new_best_model_found = False
                best_model_batch_idx = ""
