...
class DGAN:
    ...
    def _train(
        ...
        while epoch < self.config.epochs:
            ...
            for batch_idx, real_batch in enumerate(loader):
                ...
                for _ in range(self.config.discriminator_rounds):
                    ...
                    with torch.cuda.amp.autocast(enabled=True):
                        ...
                        # START :: Discriminator's loss function modification using KL Divergence
                        generated_list = generated_output.tolist()
                        real_list = real_output.tolist()
                        ...
                        # FINISH:: Discriminator's loss function modification using KL Divergence
            ...
            avg_KLdiv_loss     /= (len(loader)*self.config.discriminator_rounds)  #Averaged by batch_size and Discriminator's training steps
            avg_fft_KLdiv_loss /= (len(loader)*self.config.discriminator_rounds)  #Averaged by batch_size and Discriminator's training steps
            avg_spectral_loss  /= (len(loader)*self.config.generator_rounds)      #Averaged by batch_size and Generator's     training steps
