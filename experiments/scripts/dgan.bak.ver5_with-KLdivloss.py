...
class DGAN:
    ...
    def _train(
        ...
        def basic_sinusoidal_lr(epoch, lr_start=0.001, lr_stop=0.00001, oscillation_cycles=30, decay_rate=0.1):
        ...
        while epoch < self.config.epochs:
            ...
            generator_loss, discriminator_loss, total_gendis_loss, avg_KLdiv_loss = 0.0, 0.0, 0.0, 0.0
            ...
            for batch_idx, real_batch in enumerate(loader):
                ...
                for _ in range(self.config.discriminator_rounds):
                    ...
                    with torch.cuda.amp.autocast(enabled=True):
                        ...
                        loss = (
                              (loss_generated if self.KLdiv_loss_replacement==False else KL_divergence_loss)
                            + (loss_real      if self.KLdiv_loss_replacement==False else KL_divergence_loss)
            ...
            logger.info(f'G loss {generator_loss:.4e}, D loss {discriminator_loss:.4e}, |G|+|D| loss {total_gendis_loss:.4e}, KLdiv loss {avg_KLdiv_loss:.4e}')
